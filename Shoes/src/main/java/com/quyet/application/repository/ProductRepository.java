...
import com.quyet.application.model.dto.ChartDTO;
...
//    List<ProductInfoDTO> getListBestSellProducts(int limit);
    ...
    void minusOneProductTotalSold(long quantity, String productId);

    //Cộng một sản phẩm đã bán
    @Transactional
    @Modifying
    @Query(nativeQuery = true, value = "Update product set total_sold = total_sold + ?1 where id = ?2")
    void plusOneProductTotalSold(long quantity, String productId);
